...
async function getTotalBlogs({ end }: { end: End }) {
  ...
  if (end === "frontend") {
    const blogs = await fetch(process.env.URL ?? 'https://' + process.env.VERCEL_URL + "/api/blogs/total");
...
async function getBlogsForEmail({ email, end }: { email: string; end: End }) {
  ...
  if (end === "frontend") {
    try {
      const blogs = await fetch(process.env.URL ?? 'https://' + process.env.VERCEL_URL + "/api/blogs/totalEmail", {
        method: "POST",
